...
[**Windows**](https://drive.google.com/file/d/1z6aBaNWWxRrNhENaVSR9ixiFcsrICBWN/view?usp=sharing) (.zip, 258 MB) *Note: tested on Windows 10 Home*

[**Linux**](https://drive.google.com/file/d/1GFr8gMaq3sFArOY_IVb0MWst0DMTo6Lc/view?usp=sharing) (.zip, 253 MB) *Note: tested on Ubuntu 22.04 LTS*

[**MacOS**](https://drive.google.com/file/d/1U6kRJ04InJl_vKF2UwcpZa5_sFWqqQxo/view?usp=sharing) (.zip, 265 MB) *Note: tested on M3 Max Apple Silicon Chip*
...
##### eye_parameters:
###### pupil_diameter_mm_range: min and max pupil diameter, in **millimeters**. Sampled per frame from a clamped uniform distribution. Supported range **2.5 mm – 8.7 mm** (`PUPIL_MM_MIN`..`PUPIL_MM_MAX` in `Assets/EyeSizeCalibration.cs`); values outside this range are clamped at JSON load and a warning is logged. Internally converted to the shader's signed `_PupilSize` UV-multiplier via a closed-form analytical mapping derived directly from the shader UV math — no linear approximation, no polynomial fit. The mapping is implemented in `EyeSizeCalibration.PupilDiameterMmToPupilSize` (5000-step radial scan + 60-iteration bisection); see `docs/iris-pupil-mm-calibration.md` for the derivation and `docs/figures/pupil-mm-vs-pupilsize.png` for the curve.
